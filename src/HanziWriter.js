...
HanziWriter.prototype.updateColor = function(
  colorName,
  colorVal,
  options = {}
) {
  return this._withData(() => {
    ...
    let mutation = characterActions.updateColor(
      colorName,
      mappedColor,
      duration
    );
...
HanziWriter.prototype.setCharacter = function(char) {
  ...
  this._withDataPromise = this._loadingManager
    ...
    .then(pathStrings => {
      ...
      this._hanziWriterRenderer = new HanziWriterRenderer(
        this._character,
        this._positioner
      );
...
HanziWriter.prototype._setupListeners = function() {
  ...
  // TODO: fix
  this.addEventListener('mouseup', () => this._forwardToQuiz('endUserStroke'));
  this.addEventListener('touchend', () => this._forwardToQuiz('endUserStroke'));
};
...
HanziWriter.getScalingTransform = (width, height, padding = 0) => {
  ...
  return {
    ...
    transform: trim(`
      translate(${positioner.getXOffset()}, ${positioner.getHeight() -
      positioner.getYOffset()})
